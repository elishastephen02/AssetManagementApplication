...
namespace AssetManagement.Controllers
{
    ...
    public class ImportController : Controller
    {
        ...
        public async Task<IActionResult> Upload(DbUploadViewModel model)
        {
            ...
            {
                ModelState.AddModelError("", "Please select a database.");
                return View(model);
            }

            var extension = Path.GetExtension(model.DbFile.FileName).ToLower();

            if (extension != ".db3")
            {
                ModelState.AddModelError("", "Only .db3 files are supported.");
                return View(model);
            }
            ...
            foreach (var table in ImportMapping.Tables)
            {
                if (!adapter.TableExists(table.Key))
                    continue;

                var data = adapter.GetMappedTable(
                    table.Key,
                    table.Value);
